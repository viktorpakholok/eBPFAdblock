...
def add_cache_entries(cache, names):
    ...
    for i, name in enumerate(names):
        ...
        kb = (c_ubyte * K).from_buffer_copy(b); key_bufs.append(kb); Keys[i].p = kb
        vb = (c_ubyte * 4).from_buffer_copy(b'\0'*4); val_bufs.append(vb); Leafs[i].p = vb
...
def add_domains_batched(given_domains, batch_size):
  ...
  for i in range(0, total_domains_count, batch_size):
    ...
    if added_domains_count % batch_size * 10 == 0:
      print(f"added domains: {added_domains_count:<10} {round(added_domains_count/total_domains_count * 100, 2):>4}%", end="\r", flush=True)
...
def add_domains(given_domains):
  ...
  for domain in given_domains:
    if added_domains_count % 10000 == 0:
      print(f"added domains: {added_domains_count} {round(added_domains_count/total_domains_count * 100, 2):>8}%", end="\r", flush=True)
